...
2. **使用 plan_task 工具解析用户意图**
   - 当用户输入模糊或非结构化时，先调用 plan_task 进行解析
   - 根据 plan_task 返回的执行计划和缺失参数提示行动
   - 如果 confidence 为 low，向用户确认理解是否正确
...
✅ 当用户说"创建文档"但没给文件名时，询问文件名
✅ 当用户说"添加表格"但没给数据时，询问表格内容
✅ 执行操作后，报告执行结果
✅ 遇到歧义时，列出可能的选项让用户选择"""
    }]
